...
/**
 * Android Native x86 adapter implementation using native Android secure random APIs
 * with comprehensive error handling, syscall support, and fallback mechanisms.
 *
 * Uses x86-specific getrandom() syscall (syscall #355) with 32-bit UInt types
 * and fallback to /dev/urandom for maximum security and compatibility.
 */
internal class AndroidNativeX86SecureRandomAdapter private constructor() : SecureRandom {

    private val logger = Logger.withTag("AndroidNativeX86SecureRandomAdapter")

    companion object {
        // getrandom() syscall number for x86 Android Native
        private const val SYS_GETRANDOM = 355
        private const val GRND_NONBLOCK = 0x0001

        /**
         * Creates a new AndroidNativeX86SecureRandomAdapter instance with proper initialization.
         */
        fun create(): SecureRandomResult<AndroidNativeX86SecureRandomAdapter> {
            return SecureRandomResult.runCatching {
                val adapter = AndroidNativeX86SecureRandomAdapter()
                adapter.logger.i { "Creating Android Native x86 SecureRandom adapter..." }

                // Test initialization with diagnostic checks
                adapter.performInitializationChecks()

                adapter.logger.i { "Android Native x86 SecureRandom adapter initialized successfully" }
                adapter
            }
        }
    }
    ...
    private fun performInitializationChecks() {
        logger.i { "Performing Android Native x86 environment diagnostic checks..." }
        ...
        // Test getrandom() availability for x86
        try {
            logger.i { "Testing getrandom() syscall availability (x86 #355)..." }
            val testBytes = ByteArray(1)
            val available = tryGetrandomDiagnostic(testBytes)
            if (available) {
                logger.i { "✅ getrandom() syscall is available and working on x86" }
            } else {
        ...
        logger.i { "🎯 All Android Native x86 environment checks passed!" }
    }
    ...
    private fun tryGetrandomDiagnostic(bytes: ByteArray): Boolean {
        return memScoped {
            ...
            try {
                logger.d { "Attempting getrandom() syscall (x86 #355) with ${bytes.size} bytes..." }
                ...
                when {
                    result == bytes.size.toInt() -> {
                        logger.d { "✅ getrandom() successful on x86" }
                        return true
                    }
        ...
        return ParameterValidation.validateAndExecute {
            ...
            try {
                ...
                throw SecureRandomGenerationException(
                    "Failed to generate random bytes on Android Native x86",
                    e
                )
            }
        }
     ...
     * Android Native x86-specific APIs with proper error handling and fallback mechanisms.
     */
    private fun fillBytesSecurely(bytes: ByteArray) {
        logger.v { "Filling ${bytes.size} bytes securely on x86..." }

        try {
            // Try getrandom() syscall first (x86 syscall #355)
            logger.v { "Attempting getrandom() syscall (x86 #355)..." }
...
actual fun createSecureRandom(fallbackPolicy: FallbackPolicy): SecureRandomResult<SecureRandom> {
    // Android Native X86 uses getrandom() → /dev/urandom and has secure fallbacks only
...
actual fun createSecureRandom(): SecureRandomResult<SecureRandom> {
    ...
    logger.i { "Creating Android Native x86 SecureRandom..." }

    return try {
        val result = AndroidNativeX86SecureRandomAdapter.create()
        when (result) {
            is SecureRandomResult.Success -> {
                logger.i { "✅ Android Native x86 SecureRandom created successfully" }
                SecureRandomResult.success(result.value as SecureRandom)
            }
            is SecureRandomResult.Failure -> {
                logger.e { "❌ Failed to create Android Native x86 SecureRandom: ${result.exception.message}" }
                SecureRandomResult.failure(result.exception)
            }
        }
    } catch (e: Exception) {
        logger.e(e) { "❌ Exception during Android Native x86 SecureRandom creation" }
        SecureRandomResult.failure(
            SecureRandomInitializationException("Android Native x86 SecureRandom creation failed", e)
        )
    }
}