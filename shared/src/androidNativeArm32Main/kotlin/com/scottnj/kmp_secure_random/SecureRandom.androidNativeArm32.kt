...
/**
 * Android Native ARM32 adapter implementation using native Android secure random APIs
 * with comprehensive error handling, syscall support, and fallback mechanisms.
 *
 * Uses ARM32-specific getrandom() syscall (syscall #384) with 32-bit UInt types
 * and fallback to /dev/urandom for maximum security and compatibility.
 */
internal class AndroidNativeArm32SecureRandomAdapter private constructor() : SecureRandom {

    private val logger = Logger.withTag("AndroidNativeArm32SecureRandomAdapter")

    companion object {
        // getrandom() syscall number for ARM32 Android Native
        private const val SYS_GETRANDOM = 384
        private const val GRND_NONBLOCK = 0x0001

        /**
         * Creates a new AndroidNativeArm32SecureRandomAdapter instance with proper initialization.
         */
        fun create(): SecureRandomResult<AndroidNativeArm32SecureRandomAdapter> {
            return SecureRandomResult.runCatching {
                val adapter = AndroidNativeArm32SecureRandomAdapter()
                adapter.logger.i { "Creating Android Native ARM32 SecureRandom adapter..." }

                // Test initialization with diagnostic checks
                adapter.performInitializationChecks()

                adapter.logger.i { "Android Native ARM32 SecureRandom adapter initialized successfully" }
                adapter
            }
        }
    }
    ...
    private fun performInitializationChecks() {
        logger.i { "Performing Android Native ARM32 environment diagnostic checks..." }
        ...
        // Test getrandom() availability for ARM32
        try {
            logger.i { "Testing getrandom() syscall availability (ARM32 #384)..." }
            val testBytes = ByteArray(1)
            val available = tryGetrandomDiagnostic(testBytes)
            if (available) {
                logger.i { "✅ getrandom() syscall is available and working on ARM32" }
            } else {
        ...
        logger.i { "🎯 All Android Native ARM32 environment checks passed!" }
    }
    ...
    private fun tryGetrandomDiagnostic(bytes: ByteArray): Boolean {
        return memScoped {
            ...
            try {
                logger.d { "Attempting getrandom() syscall (ARM32 #384) with ${bytes.size} bytes..." }
                ...
                when {
                    result == bytes.size.toInt() -> {
                        logger.d { "✅ getrandom() successful on ARM32" }
                        return true
                    }
        ...
        return ParameterValidation.validateAndExecute {
            ...
            try {
                ...
                throw SecureRandomGenerationException(
                    "Failed to generate random bytes on Android Native ARM32",
                    e
                )
            }
        }
     ...
     * Android Native ARM32-specific APIs with proper error handling and fallback mechanisms.
     */
    private fun fillBytesSecurely(bytes: ByteArray) {
        logger.v { "Filling ${bytes.size} bytes securely on ARM32..." }

        try {
            // Try getrandom() syscall first (ARM32 syscall #384)
            logger.v { "Attempting getrandom() syscall (ARM32 #384)..." }
...
actual fun createSecureRandom(fallbackPolicy: FallbackPolicy): SecureRandomResult<SecureRandom> {
    // Android Native ARM32 uses getrandom() → /dev/urandom and has secure fallbacks only
...
actual fun createSecureRandom(): SecureRandomResult<SecureRandom> {
    ...
    logger.i { "Creating Android Native ARM32 SecureRandom..." }

    return try {
        val result = AndroidNativeArm32SecureRandomAdapter.create()
        when (result) {
            is SecureRandomResult.Success -> {
                logger.i { "✅ Android Native ARM32 SecureRandom created successfully" }
                SecureRandomResult.success(result.value as SecureRandom)
            }
            is SecureRandomResult.Failure -> {
                logger.e { "❌ Failed to create Android Native ARM32 SecureRandom: ${result.exception.message}" }
                SecureRandomResult.failure(result.exception)
            }
        }
    } catch (e: Exception) {
        logger.e(e) { "❌ Exception during Android Native ARM32 SecureRandom creation" }
        SecureRandomResult.failure(
            SecureRandomInitializationException("Android Native ARM32 SecureRandom creation failed", e)
        )
    }
}